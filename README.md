...
- The **Network** page in the sidebar is the full view: uptime and outages, every connected client with its IP and hostname (filterable), a join/leave activity feed, a router card with firmware and external-IP history, a selectable window (6h/24h/3d/7d), and a router restart button behind a confirmation. Per-device bandwidth is not shown because the Netgear integration only reports gateway-wide throughput — there is no per-client data to display.
- The same information also reports **internet uptime and outages**, merged from the Orbi gateway's `wan_status` sensor, gaps in its throughput history (which catch drops the polled sensor slept through, including ones where Home Assistant lost the router entirely), and external-IP changes that prove a WAN session reconnected. One drop seen by two signals is counted once. Because the gateway is polled about every 30 seconds, outages shorter than that can go unrecorded — the panel reports its own resolution and the window it actually observed rather than implying finer precision, and drops under five seconds are tallied separately as "blips". Adding Home Assistant's `ping` integration against an external host would lower that floor, since it is event-driven rather than polled. Connectivity sensors belonging to individual devices are ignored on purpose: a gadget dropping off wifi is not an internet outage.
...
**Day and night are two photographs, not one photograph and a shadow.** The sunlit half is NASA's [Blue Marble](https://visibleearth.nasa.gov/images/74218/december-blue-marble-next-generation); the dark half is NASA's Black Marble, the same globe photographed at night, so night reads as cities lit up in the dark rather than as a black hole in the map. The boundary is the real terminator — computed from the solar declination for today's date and the subsolar longitude for this minute, drawn as a path and blurred into a twilight band — so in August the Arctic stays lit around the clock and Antarctica stays dark, and the curve tilts back the other way in January.
...
- **Track a Flight** — type a flight number (IATA or ICAO) to pin it, up to six at once. Every pinned flight gets its own card: route, progress, scheduled/actual times, a delay badge, and telemetry. The currently tracked flights also rotate through the banner at the top of the header.
- **Route map** — as soon as a pinned flight has both endpoints resolved, the top half of the page becomes a map of the route: the great circle drawn on real [CARTO](https://carto.com/) dark tiles, origin and destination marked in the dashboard's own accent and warn tones, and the aircraft on the stretch it has already flown (solid) with the rest still ahead of it (dashed). The map fits its own zoom to the route, so a hop across Texas and a transpacific leg are both legible, and it follows the great circle rather than a straight line — a San Francisco–Shanghai flight arcs past the Aleutians, across the date line, the way it actually flies. With several flights pinned the map cycles through them every 20 seconds; tap a card to hold it there. Coordinates come from the same adsbdb airport records the route text already uses (`fromLat`/`fromLon`/`toLat`/`toLon` on `GET /api/flights/track`).
- **While nothing is pinned**, the top half runs a widebody showcase instead — 747, A380, 777, 787, A330, A340, 767, MD-11 — over a radar-style stage, using aircraft icons by [ADS-B Radar](https://adsb-radar.com) for macOS ([App Store](https://apps.apple.com/app/id1538149835)).
...
- `GET /api/insights/network?hours=24` — hourly average download/upload speed in Mbps, a connected-device count reconstructed from `device_tracker` history, and a `connectivity` block carrying uptime percentage, outage/blip counts, the individual outage spans with the signals that saw each one, external-IP changes, and the resolution and observed window behind those figures (see **Every tile tells a story** above)
- `GET /api/insights/clients?hours=24` — the clients currently on the network (IP, MAC, hostname, how long connected), a join/leave timeline from `device_tracker` history with Home Assistant's restart bursts filtered out, and the router's firmware version. Powers the **Network** page; kept separate from `/insights/network` because replaying every tracker's history is a much larger request than the speed summary
